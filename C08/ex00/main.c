...
/*   Updated: 2026/03/25 20:29:32 by rceschel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	throw_argc_error(int ac)
{
	if (ac < 2)
	{
		write(STDERR_FILENO, FILENAME_MISSING,
			ft_strlen(FILENAME_MISSING));
		return (1);
	}
	else if (ac > 2)
	{
		write(STDERR_FILENO, TOO_MANY_ARG,
			ft_strlen(TOO_MANY_ARG));
		return (1);
	}
	return (0);
...
int throw_cannot_read_error(int fd)
{
	if (fd == -1)
	{
		write(STDERR_FILENO, CANNOT_READ,
			ft_strlen(CANNOT_READ));
		return (1);
	}
	return (0);
}
